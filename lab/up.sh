...
write_nginx_conf() {
    ...
    mkdir -p "$dir/www/upload" "$dir/logs" "$dir/body"
    printf 'real%s %s\n' "$n" "$(real_ip "$n")" >"$dir/www/index.html"
    # A response far larger than one frame (lab/mtucheck.sh). Under DSR the
    # response goes real -> client directly; the encapsulated direction is the
    # client's, which the PUT /upload/ location below exercises.
    head -c 102400 /dev/urandom >"$dir/www/100k.bin"
    chmod -R a+rX "$dir"
    chmod 1777 "$dir/www/upload" "$dir/body"   # nginx workers do not run as root
...
http {
    ...
    server {
        ...
        root $dir/www;
        index index.html;
        # PUT /upload/<name> stores the body, so nginx reads the whole request:
        # the client-to-VIP direction, the one PacketBalance encapsulates, carries
        # full-size segments (the MTU check in lab/mtucheck.sh).
        location /upload/ {
            dav_methods PUT;
